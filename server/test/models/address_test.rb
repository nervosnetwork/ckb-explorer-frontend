...
class AddressTest < ActiveSupport::TestCase
  ...
  test "should update the related address's ckb_transactions_count after block synced" do
    VCR.use_cassette("blocks/10") do
      SyncInfo.local_inauthentic_tip_block_number
      address = create(:address, address_hash: "0xcb7bce98a778f130d34da522623d7e56705bddfe0dc4781bd2331211134a19a5", ckb_transactions_count: 1)
      ...
      VCR.use_cassette("blocks/three", match_requests_on: [:body], record: :new_episodes) do
        ...
        updated_balances =
          local_block.contained_addresses.map do |address|
            CKB::Utils.get_balance(address.address_hash)
          end
    ...
    VCR.use_cassette("genesis_block") do
      VCR.use_cassette("blocks/three", match_requests_on: [:body], record: :new_episodes) do
        ...
        updated_cell_consumed =
          local_block.contained_addresses.map do |address|
            CKB::Utils.address_cell_consumed(address.address_hash)
          end
