  ...
  class PersistTest < ActiveSupport::TestCase
    ...
    test ".save_block created block's attribute value should equal with the node block's attribute value" do
      VCR.use_cassette("blocks/10") do
        ...
        local_block = CkbSync::Persist.save_block(node_block, "inauthentic")
        local_block_hash = local_block.attributes.select { |attribute| attribute.in?(%w(difficulty block_hash number parent_hash seal timestamp transactions_root txs_proposal uncles_count uncles_hash version witnesses_root proposals)) }
        ...
        local_uncle_blocks =
          local_block.uncle_blocks.map do |uncle_block|
            uncle_block = uncle_block.attributes.select do |attribute|
              attribute.in?(%w(difficulty block_hash number parent_hash seal timestamp transactions_root txs_proposal uncles_count uncles_hash version witnesses_root proposals))
            end
    ...
    test ".save_block generated block's miner hash should be nil when binary hash is invalid" do
      VCR.use_cassette("blocks/10") do
        SyncInfo.local_inauthentic_tip_block_number
        node_block = CkbSync::Api.instance.get_block(DEFAULT_NODE_BLOCK_HASH).deep_stringify_keys
        set_default_lock_params(node_block: node_block, binary_hash: "0xc30257c81dde7766fc98882ff1e9f8e95abbe79345982e12c6a849de90cbbad1")
    ...
    test ".save_block generated block should has correct miner hash when miner use default lock script " do
      VCR.use_cassette("blocks/10") do
        SyncInfo.local_inauthentic_tip_block_number
        node_block = CkbSync::Api.instance.get_block(DEFAULT_NODE_BLOCK_HASH).deep_stringify_keys
        set_default_lock_params(node_block: node_block, binary_hash: LockScript::SYSTEM_SCRIPT_CELL_HASH)
