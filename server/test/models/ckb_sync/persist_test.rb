  ...
  class PersistTest < ActiveSupport::TestCase
    ...
    test ".save_block created block's attribute value should equal with the node block's attribute value" do
      VCR.use_cassette("blocks/10") do
        ...
        local_block = CkbSync::Persist.save_block(node_block, "inauthentic")
        local_block_hash = local_block.attributes.select { |attribute| attribute.in?(%w(difficulty block_hash number parent_hash seal timestamp txs_commit txs_proposal uncles_count uncles_hash version witnesses_root proposals)) }
        ...
        local_uncle_blocks =
          local_block.uncle_blocks.map do |uncle_block|
            uncle_block = uncle_block.attributes.select do |attribute|
              attribute.in?(%w(difficulty block_hash number parent_hash seal timestamp txs_commit txs_proposal uncles_count uncles_hash version witnesses_root proposals))
            end
