...
export const shannonToCkbDecimal = (value: BigNumber | string | number, decimal: number): string => {
  if (decimal < 0) {
    throw Error('Decimal must be bigger than zero')
  }
  if (!value) return '0'
  ...
  return (Math.floor(num.toNumber() * 10 ** decimal) / 10 ** decimal).toString()
}
