...
const AddressTitle = ({ lockHash }: { lockHash: string }) => {
...
export default (props: React.PropsWithoutRef<RouteComponentProps<{ hash: string }>>) => {
  ...
  const getAddressInfo = () => {
    ...
    fetchAddressInfo(lockHash)
      .then(json => {
        appContext.hideLoading()
        const { data, error } = json as Response<AddressWrapper>
        if (error) {
          browserHistory.push(`/search/fail?q=${lockHash}`)
        } else {
  ...
  const getTransactions = (page_p: number, size_p: number) => {
    appContext.showLoading()
    fetchTransactionsByAddress(lockHash, page_p, size_p)
      .then(json => {
        appContext.hideLoading()
  ...
  const onChange = (page_p: number, size_p: number) => {
    setPageNo(page_p)
    setPageSize(size_p)
    props.history.push(`/lockhash/${lockHash}?page=${page_p}&size=${size_p}`)
  }

  return (
    <Content>
      <AddressContentPanel width={window.innerWidth} className="container">
        <AddressTitle lockHash={lockHash} />
